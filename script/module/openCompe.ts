...
export async function openCompe(account: Account, name: string, songs: [CompeSong, CompeSong, CompeSong]) {
    ...
    body.set('title', '0');
    body.set('compe_days', '0');
    body.set('comment', '');
    body.set('_tckt', ticket ?? '');

    const response = await fetch("https://donderhiroba.jp/ajax/compe_create.php", {
        "headers": header,
        "body": body.toString(),
        "method": "post"
    });
